...
| UI · every target at least 44×44 | **Done** | Eleven controls were between 22 and 40 points: the saved-park remove, the terrain chips, Discover's save, the park's Save and Offline Pack, Profile's pack buttons and tint reset, the trip delete disc, the builder's day steppers and pick chips. All 44 or more. — claude |
...
| Flow 4 · select any park from the shared directory | **Done** | `TripBuilder.results` read `library.orderedParks` alone, so a trip could only be built from the curated eight — a park found on Discover could be opened and saved but not planned around. It now merges the sixty-two on the phone, the eight with day plans, and whatever the live directory finds, with the same source line Discover carries. "moab" gives five parks including Dead Horse Point; "texas" gives thirty-eight. — claude |
